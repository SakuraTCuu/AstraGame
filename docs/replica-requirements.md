...
| --- | --- | --- |
| World map | Configured continuous world and event-gated regions | Reproduce the observed connected map overview, region travel and actual unlock rules |
...
| Fog and unlocking | Locked/hidden/visible/explored state and event conditions | Confirm resource spending and progression gates in the running game; match smooth fog appearance |
...
| 2.5D presentation | Ground projection and actor Y ordering | Replace graybox presentation with project-owned visuals and compare terrain/actor occlusion |
| Automatic navigation | A*, minimap destinations, manual override and route restoration | Match quest destinations, route markers and blocked-target behavior |
...
| State machines | Tested actor cast states, surviving leader control, pause/win/fail, restart and result idempotence | Match reference death/revival and repeat the longer-session lifecycle checks |
...
Not yet confirmed: exact attack/cooldown formulas and damage-type modifiers; the meaning and count of Boss health layers; purchased fog-region costs; death/revival rules; all map layouts; path-resume timing; monster leash and respawn parameters. Current demo values remain explicitly original placeholders.

The initial `discover:` and `clear:` region rules are engineering fixtures. They must not be presented as confirmed reference-game unlocking rules. No raw reference-game code, art, audio or extracted configuration is part of the implementation.
