...
Team experience follows item 7 rewards and the configured PlayerLevel thresholds. A natural first-Boss run advanced from level 1 to 2 with 40/600 experience, and restored those values from a save. Kill counters support multi-kill and monster-subtype prerequisites. The journal adds mainline/first-kill reward claims and ordered rank promotion, with seven mainline steps and the first promotion verified through natural gameplay. Later equipment, recruitment, rename, hero-development and other-mode actions are still absent; their conditions remain enforced.
