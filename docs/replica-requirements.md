...
| State machines | Independent Buff/state timers; stun, freeze, root, silence and airborne control; separate control/displacement/interruption immunity; actor/cast/travel/run and recovery states; persisted exploration, development and respawn timers | Match airborne trajectories, fear and other specialized states, existing-control immunity interactions and exact interruption boundaries; verify revival rules, restore in-progress enemy combat and extend long-session checks |
